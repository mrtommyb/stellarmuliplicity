...
def get_spectral_type(temp):
  """ this is taken from
      ...
      Eric Mamajek (JPL, University of Rochester)
      Version 2017.02.03
  """
  if temp > 31500:
    stype = 'O'
  elif (temp <= 31500) & (temp > 9700):
    stype = 'B'
  elif (temp <= 9700) & (temp > 7200):
    stype = 'A'
  elif (temp <= 7200) & (temp > 5920):
    stype = 'F'
  elif (temp <= 5920) & (temp > 5280):
    stype = 'G'
  elif (temp <= 5280) & (temp > 3850):
    stype = 'K'
  elif (temp <= 3850) & (temp > 2450):
    stype = 'M'
  elif (temp <= 2450) & (temp > 1260):
    stype = 'L'
  elif (temp <= 1260):
    stype = 'T'
...
def kepler_noise_6h(mag):
  """
  rough approx of kepler noise level
  """
  kp_pars = [8.99783121e-12,   1.10726939e+01,   1.06404927e+01]
  return powerlaw(mag, *kp_pars)
